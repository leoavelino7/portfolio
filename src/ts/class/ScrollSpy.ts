...
export class ScrollSpy {
    ...
    */
    private _mountList(): void {
        let list: any = document.querySelectorAll(this.identifier);
     ...
     * @return Array<Number>
    */
    public nearestValue(ideals: Array<number>, value: number): Array<number> {
        let lo = -1,
            hi = ideals.length;
     ...
     * @return Array<number>
    */
    private _mountArrayNumber(list: Object): Array<number>{
        let myArray = [];
    ...
    */
    private _start(): void {
        let myListNumber = this._mountArrayNumber(this.elements),
            scrollPosition = (document.documentElement.scrollTop || document.body.scrollTop),
            arrayResult = this.nearestValue(myListNumber, scrollPosition);

        for (let id in this.elements) {
            if (this.elements[id] === arrayResult[1]) {
                let oldElement = document.querySelector(`.active`),
                    newElement: any = document.querySelector(`a[href*=${id}]`);
